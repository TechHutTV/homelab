...
## Bubble Cards 
I'm starting to utilize Bubble Cards for my dashboard. I'll add more information soon, but first my custom card for shades with set_position variables on the sub-buttons.
...
1. Sign in and create a new project selecting Smart Home for both Industry and Development Method. Next, on the API options add 'Device Log Query' as you may need it to see important information about your devices.
2. Within the project overview page select 'Devices' and then 'Link App Account'. Use the scanner in the settings of your Tuya app to scan the QR code. Once you authenticate your devices should appear under 'All Devices'. If you see Read under 'Device Permission' you need to change this so it's listed as Controllable.
...
4. Within Home Assistant navigate to the newly added LocalTuya integration. Click on 'Configure' and then 'Manage Cloud API account'. Paste in all the details from your Tuya project dashboard. Click submit and you should be good to go.
...
If you need to manually connect to a device, it's important to know where to find your local ids and values. These can be found within your project on the Tuya IoT Platform. For steps within the API Explorer copy the Device ID found under All Devices.
...
* min/max values: Devices > All Devices > Debug Device > Device Debugging > Standard Instruction Set

### XMCOSY String Lights with LocalTuya

#### Finding Raw Data Values for Scenes
You can use the Tuya app to create custom or access pre-existing scenes. Open the developer portal, go to devices and click on "debug device". Open the device logs page and select the 'scene' logs from the dropdown menu on the top and click search. This should provide a overview of all scenes that were activated.
